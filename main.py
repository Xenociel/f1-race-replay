...
if __name__ == "__main__":
    ...
    # Check if any command line arguments related to race selection are present
    has_args = any(arg in sys.argv for arg in ["--year", "--round", "--sprint", "--qualifying", "--sprint-qualifying"])

    if not has_args:
        # [수정된 부분] 인자가 없으면 바로 메인 메뉴 실행 (데이터 없이 호출)
        print("No arguments provided. Launching Main Menu...")
        run_arcade_replay()
    else:
        # 인자가 있으면 기존 로직대로 특정 레이스 로드
        if "--year" in sys.argv:
